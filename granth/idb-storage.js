...
export async function idbLoad(dbName, key) {
  ...
  try {
    const value = await tx(idb, 'readonly', (s) => s.get(key));
...
export async function idbSave(dbName, key, bytes) {
  const idb = await open(dbName);
  try {
...
export async function idbRemove(dbName, key) {
  const idb = await open(dbName);
  try {
 ...
 */
export async function openIdbBackedDb(sqlite3, { dbName, key, debounceMs = 250 }) {
