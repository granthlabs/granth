...
export const col = (keyPath) => `ix$${keyPath.replace(/\./g, '$')}`;
export const jsonPath = (keyPath) => `$.${keyPath}`;

/**
 * Parse one Dexie store spec: '++id, name, &email, *tags, [first+last]'
 * The first entry is the primary key.
 */
export function parseStore(table, spec) {
...
export const shadowTable = (store, ix) => `${store.table}$${ix.name}`;

/** DDL to create a store from nothing. */
export function createStoreSql(store) {
  const t = store.table;
...
export function createIndexSql(store, ix) {
  const t = store.table;
  if (ix.multi) {
    ...
    const fill = (row) =>
      ...
      `FROM json_each(${row}."_doc", '${jsonPath(ix.keyPaths[0])}') je ` +
      // json_each on a scalar yields one row; only arrays are a multiEntry index.
      `WHERE json_type(${row}."_doc", '${jsonPath(ix.keyPaths[0])}') = 'array'`;
    return [
...
export function dropStoreSql(store) {
  const sql = [];
...
export function parseSchema(stores) {
  return Object.fromEntries(
    Object.entries(stores)
      .filter(([, spec]) => spec !== null) // Dexie: null deletes a store
      .map(([table, spec]) => [table, parseStore(table, spec)])
  );
}

/** Table names deleted in this version (spec === null). */
export const deletedStores = (stores) =>
  Object.entries(stores).filter(([, s]) => s === null).map(([t]) => t);

export function findIndex(store, name) {
...
export function indexExpr(store, ix, kpIndex = 0) {
  const kp = ix.keyPaths[kpIndex];
