 ...
 */
function multiEntryIn(store, ix, sqlCond, params) {
...
function compileCond(store, cond) {
...
export function compileWhere(store, or) {
...
function compileOrderLimit(store, plan) {
  let sql = '';
 ...
 */
export function boundIndex(store, plan) {
 ...
 */
export function compile(store, plan, mode = 'docs') {
...
export function compileDelete(store, plan) {
  const t = q(store.table);
  const pk = q(store.primKey.name);
  const inner = compile(store, plan, 'keys');
...
export function compileModify(store, plan, changes) {
  const t = q(store.table);
  const pk = q(store.primKey.name);
  const inner = compile(store, plan, 'keys');
  return {
