...
features:
  - icon: 🔁
    ...
    details: The API is diffed against the real dexie package by a generated audit that fails the build on regression — WhereClause 18/18. Run npx granth-codemod, then import your existing IndexedDB data.
  - icon: ⚡
    title: A real query planner
    details: Filter on one index and order by another. A cursor-based store uses one index per query; SQL has no such limit.
  - icon: 🧬
